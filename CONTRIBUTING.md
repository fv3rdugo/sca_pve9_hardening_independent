...
[search the issue tracker](https://github.com/fv3rdugo/sca_pve9_hardening_independent/issues)
to see if someone else in the community has already created a ticket.
If it is not, go ahead and [create a new issue](https://github.com/fv3rdugo/sca_pve9_hardening_independent/issues/new)!
...
A recurrent problem for newcommers is to submit code automatically cleaned by the editor. Usually, removed end of line spaces or spaces converted to tabs. Please make sure you don't do that. Run ``git diff`` before commit to see what you are exactly contributing.
...
a reason, and we should have an [issue](https://github.com/fv3rdugo/sca_pve9_hardening_independent/issues) for that, so we decided to add the issue number in the footer.
...
* **Ensure the bug was not already reported** by searching on GitHub under [Issues](https://github.com/fv3rdugo/sca_pve9_hardening_independent/issues).

* If you're unable to find an open issue addressing the problem, [open a new one](https://github.com/fv3rdugo/sca_pve9_hardening_independent/issues/new).
...
If you change a translation or language file make sure you follow this small [guide](TODO) and don't forget to add the issue number when committing.
...
Finally, go to your GitLab repository and
[create a Pull Request](https://github.com/fv3rdugo/sca_pve9_hardening_independent/pulls)
...
```sh
git remote add upstream https://github.com/fv3rdugo/sca_pve9_hardening_independent.git
git fetch upstream
...
```sh
git remote add upstream git@github.com:fv3rdugo/sca_pve9_hardening_independent.git
git fetch upstream
git checkout main
...
We create a new branch called *feature/cool_thing* and make it exactly like branch:

First we add the upstream remote source and fetch it. If you added this remote before you will get an error *fatal: remote upstream already exists.*. Don't worry and run the `git fetch upstream` anyway so it downloads the source.

```sh
git remote add upstream https://github.com/fv3rdugo/sca_pve9_hardening_independent
git fetch upstream
```
...
Reset this branch, now it will be an exact replica of upstream :
